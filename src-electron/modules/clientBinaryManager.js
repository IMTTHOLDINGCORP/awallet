...
const BINARY_URL = 'https://raw.githubusercontent.com/IMTTHOLDINGCORP/awallet/master/clientBinaries.json'
    ...
    return got(BINARY_URL, {
      ...
      .then(localConfig => {
        ...
        return mgr
          .init({
            folders: [
              path.join(Settings.userDataPath, 'binaries', 'aether', 'unpacked')
            ]
          })
          .then(() => {
          ...
          dialog.showMessageBox(
            {
              type: 'warning',
              buttons: ['OK'],
              message: global.i18n.t('awallet.errors.nodeChecksumMismatch.title'),
              detail: global.i18n.t(
                'awallet.errors.nodeChecksumMismatch.description',
                {
                  type: nodeInfo.type,
