...
class Settings {
  ...
  get chainDataPath () {
    ...
    if (this.platform === 'win') {
      dataDir = path.join(dataDir, 'AppData', 'Roaming', 'aether')
    } else if (this.platform === 'mac') {
      dataDir = path.join(dataDir, 'Library', 'aether')
    } else {
      dataDir = path.join(dataDir, '.aether')
    }
    if (this.isTestnet) {
  ...
  get chainDataDir () {
    return path.join(this.chainDataPath, 'aether', 'chaindata')
  }
