...
import aetherNode from './aetherNode'
...
class NodeSync extends EventEmitter {
  constructor () {
    ...
    aetherNode.on('state', _.bind(this._onNodeStateChanged, this))
  }
  ...
  start () {
    ...
    this._syncPromise = Q.try(() => {
      if (!(aetherNode.state === aetherNode.STATES.CONNECTED)) {
        throw new Error('Cannot sync - aetherNode node not yet connected')
      }
  ...
  _onNodeStateChanged (state) {
    switch (state) { // eslint-disable-line default-case
      ...
      case aetherNode.STATES.STOPPING:
        log.info('aether node stopping, so stop sync')

        this.stop()
        break
      // auto-sync whenever node gets connected
      case aetherNode.STATES.CONNECTED:
        log.info('aether node connected, re-start sync')
