...
import aetherNode from '../modules/aetherNode'
...
import AwalletMenu from '../modules/menu'
...
function onReady () {
  ...
  let awalletMenu = new AwalletMenu(mwin)
  awalletMenu.create()
  /** for context menus */
...
function kickStart () {
  ...
  aetherNode.on('nodeConnectionTimeout', () => {
    windows.broadcast(Types.UI_ACTION_NODE_STATUS, 'connectionTimeout')
  })

  aetherNode.on('nodeLog', data => {
    windows.broadcast(Types.UI_ACTION_NODE_LOGTEXT, data.replace(/^.*[0-9]]/, ''))
  })

  // state change
  aetherNode.on('state', (state, stateAsText) => {
    windows.broadcast(
      'uiAction_nodeStatus',
      stateAsText,
      aetherNode.STATES.ERROR === state ? aetherNode.lastError : null
    )
  })
  ...
  Q.resolve()
    ...
    .then(() => {
      return aetherNode.init()
    })
    .then(() => {
      log.info('aether node started.')
    })
